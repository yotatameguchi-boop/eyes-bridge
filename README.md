...
**着信は段階的に鳴らす。**
1人ずつの順番待ちにすると、夜間は依頼者が数十秒待たされる。かといって全員を一度に鳴らすと、
取れるのは1人だけなので、残りは「鳴ったのに出たら終わっていた」を繰り返して離れていく。
最初に5人、15秒たっても誰も取らなければ次の15人、さらに30人、と段階的に鳴らし、
誰かが取った時点で次の段は鳴らさない。同じ人を同じ依頼で2回は鳴らさない。
誰を選ぶか（審査済み・待機中・鳴らさない時間帯でない・ブロック関係に無い）は DB の
`take_ring_wave` が1つの処理で決める。段の大きさと間隔は `RING_WAVE_SIZES` / `RING_WAVE_SECONDS`。

**ボランティアは「鳴らさない時間帯」を選べる。**
「夜10時から朝7時」などから選ぶ（時刻の自由入力は、スクリーンリーダーでも手でも操作が細かくなるのでやめた）。

**アプリを開いている人にも、着信は段階どおりにだけ鳴らす。**
以前は新しい依頼が入るたびに、アプリを開いている全員の画面で着信を鳴らしており、段階化をすり抜けていた。
今はアプリの一覧に足すだけで、電話の着信画面を出すのはサーバからの着信だけ。

**待機列の知らせは Broadcast で配る。**
以前は待機中のボランティアがそれぞれ表の変更を購読しており、変更のたびに購読者一人ひとりの
権限を確かめるので、人数が増えると重くなる。今は DB が1回だけ Broadcast で配り、受け取れる人を
`realtime.messages` の権限で承認済みのボランティアに絞る。知らせには依頼者が誰かを含めない。
取られた・取り下げられた依頼の知らせも配るので、他の人が取った依頼は一覧から消え、鳴っていれば止まる。
...
# 手元では Expo / APNs に送らない（誰を選んだかの記録だけ残す）。段の間隔も縮める
RING_DRY_RUN=true
RING_WAVE_SECONDS=2
```
...
* 本物の Edge Function で、着信が5人→残り、と段階的に鳴り、鳴らさない時間帯の人は外れ、
