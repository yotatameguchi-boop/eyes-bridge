...
test("案内のあとに、読めたところを1つの読み上げで続けて読む", () => {
  // 薬の注意が入らない文で確かめる（薬の注意の順番は cautions.test.ts）
  const lines = [line([0.1, 0.3, 0.99, 0.34], 0.95, "合計 1,280円")];
  const r = composeReading("合計 1,280円", lines, frame);
  assert.match(r.spoken, /^右側が切れています。.*読めたところを読みます。合計 1,280円$/);
});
